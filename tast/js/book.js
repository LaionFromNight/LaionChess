/* LaionBook — demo opening statistics keyed by board FEN + turn,
   plus a shared "Common Moves" table renderer. */
(function () {
  /* entries: [san, played %, games, white win %, draw %] (black = remainder) */
  var BOOK = {
    /* start position */
  ...
  function render(container, key, onPlay) {
    container.innerHTML = '';
    var rows = BOOK[key];
    var head = document.createElement('div');
    head.className = 'book-head';
    head.innerHTML = '<span class="m">Move</span><span class="p">%</span><span class="g">Games</span><span class="w">Winrate</span>';
    container.appendChild(head);
    if (!rows) {
      var empty = document.createElement('div');
      empty.className = 'book-empty';
      empty.textContent = 'No book data for this position.';
      container.appendChild(empty);
      return;
    }
    rows.forEach(function (r) {
      var san = r[0], pct = r[1], games = r[2], ww = r[3], dd = r[4], bb = 100 - r[3] - r[4];
      var b = document.createElement('button');
      b.className = 'book-row';
      b.type = 'button';
      b.innerHTML =
        '<span class="m">' + san + '</span>' +
        '<span class="p">' + pct + '%</span>' +
        '<span class="g">' + games + '</span>' +
        '<span class="bar"><i class="bw" style="width:' + ww + '%">' + ww + '%</i>' +
        '<i class="bd" style="width:' + dd + '%"></i>' +
        '<i class="bb" style="width:' + bb + '%">' + bb + '%</i></span>';
      if (onPlay) b.addEventListener('click', function () { onPlay(san); });
      container.appendChild(b);
    });
  }

  window.LaionBook = { data: BOOK, render: render };
})();
